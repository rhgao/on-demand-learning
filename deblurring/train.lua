...
opt = {
   ...
   name = 'deblur',        -- name of the experiment you are running
...
-- Construct sigma table according to training design of batches
local sigma_table = {0.5,1.5,2.5,3.5,4.5}
local batch_sigma = {}
...
local fGx = function(x)
   ...
   for i = 1, opt.batchSize do
      --blur three channels separately
      sigma_X = torch.uniform(batch_sigma[i]-0.5,batch_sigma[i]+0.5)
      sigma_Y = torch.uniform(batch_sigma[i]-0.5,batch_sigma[i]+0.5)
      --randomly sample a kernelSize
      local kernelSize = math.random(1,9) * 2 + 1
      for j = 1, 3 do
        local image_blurred = cv.GaussianBlur{corrupted_image[{{i},{j},{},{}}]:squeeze(),ksize=kernelSize,sigmaX=sigma_X,sigmaY=sigma_Y}
        corrupted_image[{{i},{j},{},{}}]:copy(image_blurred)
      end
   end
   ...
   -- backpropogation to update weights
